...
export default function MisPagosPendientes() {
  ...
  const totalMontoAcumulado = deudas.reduce(
    (sum, d) => sum + Number(d.montoTotalPagar || 0),
    0,
  );
                ...
                deudas.map((d, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`px-4 py-3 font-mono text-xs ${esVencido ? "text-red-400 font-bold" : "text-gray-400"}`}
                      >
                        {fechaFormateada}
                      </td>
                      ...
                      <td
                        className={`px-4 py-3 font-mono ${tieneMora ? "text-red-400 font-semibold" : "text-gray-500"}`}
                      >
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() =>
                            toast.info(
                              `Para pagar S/. ${Number(d.montoTotalPagar).toFixed(2)}, acércate a Tesorería e indica tu dni y el concepto o el ID de Deuda: #${d.idDeuda}`,
                              { autoClose: 6000 },
                            )
                          }
      ...
      {!cargando && deudas.length > 0 && (
        <div className="mt-4 text-right">
          <span className="text-xs text-gray-400 bg-[#111e30] border border-[#1e3a5f] px-3 py-1.5 rounded-lg">
            ...
            <strong className="text-white">{deudas.length}</strong>
          </span>
        </div>
      )}
    </div>
