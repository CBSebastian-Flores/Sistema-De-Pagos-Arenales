...
export default function ConfiguracionObligacion() {
    ...
    const cargarSocios = async () => {
      try {
        ...
        const activos = res.data.filter(
          (u) => u.tipoRol === "Socio" && u.estado === true
        )
        setSocios(activos.length)
      } catch {
        toast.error("No se pudo calcular la cantidad de socios activos")
      } finally {
  ...
  const validar = () => {
    ...
    if (!fechaVencimiento) nuevosErrores.fechaVencimiento = "Selecciona la fecha de vencimiento"
  ...
  const handleGenerar = async () => {
    ...
    try {
      const payload = {
        ...
        montoBase: cuotaPorSocio,
        fechaVencimiento,
        // Si es variable, mandamos también el monto de la factura recibida como referencia
        ...(esVariable && { facturaTotal: Number(facturaTotal) }),
      }

      // Endpoint a confirmar con backend para generación masiva de deudas
      await api.post("/api/deudas/generar-masivo", payload)
      toast.success(`Obligación generada para ${socios} socio${socios !== 1 ? "s" : ""}`)
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-[#111e30] border border-[#1e3a5f] rounded-2xl p-8 flex flex-col gap-7">
          ...
          <div className="flex flex-col gap-2">
            ...
            {/* Badge de modalidad, anidado bajo el selector que lo origina */}
          ...
          {/* Bloque de montos: factura + cuota, agrupados y separados del resto */}
          {servicioSeleccionado && (
            <div className="flex flex-col gap-5 pt-7 border-t border-[#1e3a5f]/60">
              ...
              {esVariable && (
                <div className="flex flex-col gap-2">
                  ...
                  />
                  {errores.facturaTotal && <p className="text-red-400 text-xs">{errores.facturaTotal}</p>}
                </div>
              )}

              {/* Cuota por Socio — siempre visible una vez seleccionado el servicio, siempre bloqueada */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                  Cuota por Socio (S/.)
                </label>
          ...
          {/* Fecha de vencimiento, separada como el cierre del formulario */}
          <div className="flex flex-col gap-2 pt-7 border-t border-[#1e3a5f]/60">
            ...
            />
            {errores.fechaVencimiento && <p className="text-red-400 text-xs">{errores.fechaVencimiento}</p>}
          </div>
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-lg transition-colors text-sm"
          >
