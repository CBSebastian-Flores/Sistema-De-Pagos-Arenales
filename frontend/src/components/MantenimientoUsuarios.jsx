...
function Modal({ titulo, onClose, children, ancho = "max-w-lg" }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div
        className={`relative w-full ${ancho} mx-4 bg-[#111e30] border border-[#1e3a5f] rounded-2xl shadow-2xl`}
      >
        {/* Header */}
    ...
    <Modal titulo="Editar Usuario" onClose={onClose} ancho="max-w-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <CampoForm
          label="Nro. Puesto"
          nombre="nroPuesto"
          valor={form.nroPuesto}
          onChange={handleChange}
          error={errores.nroPuesto}
          maxLength={3}
        />
        ...
        >
          ...
          {errores.genero && (
            <p className="text-red-400 text-xs">{errores.genero}</p>
          )}
        </CampoForm>
        ...
        >
          ...
          {errores.idRol && (
            <p className="text-red-400 text-xs">{errores.idRol}</p>
          )}
        </CampoForm>
      </div>
...
function ModalRestablecerPassword({ usuario, onClose }) {
  ...
  const handleChange = (nombre, valor) => {
    ...
    if (nombre === "confirmarContrasena") {
      const err =
        valor !== form.contrasena ? "Las contraseñas no coinciden" : null;
  ...
  return (
    <Modal
      titulo={`Restablecer Contraseña — ${usuario.nombres}`}
      onClose={onClose}
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-gray-400">
          Establece una nueva contraseña para el usuario. Deberá tener al menos
          8 caracteres.
        </p>
        <CampoPassword
...
export default function MantenimientoUsuarios() {
  ...
  const abrirModalAuditoria = (usuario, accion) => {
  ...
  // ── Eliminar lógico ──
  const handleCambioEstado = async (motivo) => {
    try {
      if (!usuarioSeleccionado) return;

      if (accionPendiente === "inhabilitar") {
        await inhabilitarUsuario(usuarioSeleccionado.idUsuario, motivo);
        toast.success("Usuario inhabilitado correctamente");
      } else {
        await habilitarUsuario(usuarioSeleccionado.idUsuario, motivo);
    ...
    <div className="p-6 min-h-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">
          Gestión centralizada del personal y comerciantes del CC Arenales
        </p>
      </div>
            ...
            <tbody>
              {cargando ? (
                ...
                usuariosFiltrados.map((u, i) => (
                  <tr
                    key={u.idUsuario}
                    className={`border-b border-[#1e3a5f]/50 transition-colors hover:bg-[#1a2d4a]/40 ${i % 2 === 0 ? "" : "bg-[#0f1b2d]/30"}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <BadgeEstado
                        activo={u.estado === true || u.estado === 1}
                      />
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-amber-400 hover:bg-amber-500/15 hover:text-amber-300 transition-colors cursor-pointer"
                        >
                          <IconKey />
                        </button>
                        ...
                        {u.estado === true || u.estado === 1 ? (
                          <button
                            onClick={() =>
                              abrirModalAuditoria(u, "inhabilitar")
                            }
                          ...
                          <button
                            onClick={() => abrirModalAuditoria(u, "habilitar")}
      ...
      <ModalAuditoria
        ...
        mensajeAdvertencia={
          accionPendiente === "inhabilitar"
            ? "Esta acción cambiará el estado del usuario a Inactivo. Debe registrar una justificación."
            : "Esta acción cambiará el estado del usuario a Activo. Debe registrar una justificación."
        }
        onClose={() => {
          setModalAuditoria(false);
          setUsuarioSeleccionado(null);
