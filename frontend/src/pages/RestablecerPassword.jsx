...
export default function RestablecerPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 🔑 LEER MENSAJE DE ÉXITO: Muestra el mensaje dinámico del Backend (HTTP 200)
      toast.success(
        response.data?.mensaje || "Contraseña actualizada correctamente",
      );
      navigate("/login");
    } catch (error) {
      // 🔑 LEER MENSAJE DE ERROR: Extrae el JSON que configuramos en el catch de Java (HTTP 400)
      const mensajeError =
  ...
  if (!token) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0f1e35]">
        <div className="bg-[#1a2d4a] rounded-3xl p-8 w-full max-w-md text-center">
          <h1 className="text-2xl font-bold text-red-400 mb-4">
            Enlace Inválido o Expirado
          </h1>
          <p className="text-gray-300 text-sm mb-6">
            El token de recuperación no se encuentra en la URL o ya no es
